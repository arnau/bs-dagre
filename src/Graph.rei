...
/** Adds a node with its layout attributes to the graph.
    ...
    Note that the JavaScript version makes the attributes (i.e. [label] in
    dagre parlance) optional. This interface uses two functions: [setNode] and
    [setNodeWith]. If you use the former, make sure the graph has a default
    set of attributes for new nodes with [setDefaultNodeAttrs].
...
/** Returns the predecessor nodes for the given node. Returns [None] if the
    ...
    Note that undirected graphs have undefined behaviour. Use
    [Graph.neighbors] instead.
...
/** Adds an edge with its layout attributes to the graph.
    ...
    Note that the JavaScript version makes the attributes (i.e. [label] in
    dagre parlance) optional. This interface uses two functions: [setEdge] and
    [setEdgeWith]. If you use the former, make sure the graph has a default
    set of attributes for new nodes with [setDefaultNodeAttrs].
